...
with col2:
    if request:
        response = client.chat.completions.create(
            model="gpt-4o", 
            messages=[
                {"role": "system", "content": f"You are a legal assistant. You answer questions about the following contract: {contract}"},
    ...
    if st.button('Generate template'):
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Generate a Service Delivery Agreement with the following elements: Service Provider: {service_provider}, Client: {client}, Description of Services: {services_description}, Start Date: {start_date}, Duration: {duration}" },
            ]
        )
